...
var editAllianceBasicInfo = function(name, tag, language, flag, callback){
	var info = {
...
var editAllianceTerrian = function(terrain, callback){
	var info = {
		terrain:terrain
	}
	var route = "logic.playerHandler.editAllianceTerrian"
	pomelo.request(route, info, callback)
}
...
var requestToSpeedUp = function(eventType, eventId, callback){
	var info = {
		eventType:eventType, eventId:eventId
	}
	var route = "logic.playerHandler.requestToSpeedUp"
	pomelo.request(route, info, callback)
}
...
var upgradeAllianceBuilding = function(buildingName, callback){
	var info = {
		buildingName:buildingName
	}
	var route = "logic.playerHandler.upgradeAllianceBuilding"
	pomelo.request(route, info, callback)
}

var upgradeAllianceVillage = function(villageType, callback){
	var info = {
		villageType:villageType
	}
	var route = "logic.playerHandler.upgradeAllianceVillage"
	pomelo.request(route, info, callback)
}
	...
	describe("playerHandler", function(){
		...
		it("createHouse 小屋数量超过限制", function(done){
			var createHouse = function(buildingLocation, houseLocation, callback){
				var houseInfo = {
					buildingLocation:buildingLocation, houseType:"dwelling", houseLocation:houseLocation, finishNow:true
				}
				var route = "logic.playerHandler.createHouse"
			...
			var clearEvents = function(callback){
				var chatInfo = {
					text:"rmbuildingevents", type:"global"
				}
				var route = "chat.chatHandler.send"
		...
		it("upgradeHouse 主体建筑必须大于等于1级", function(done){
			var houseInfo = {
				buildingLocation:5, houseLocation:1, finishNow:false
			}
			var route = "logic.playerHandler.upgradeHouse"
			pomelo.request(route, houseInfo, function(doc){
				doc.code.should.equal(500)
		...
		it("upgradeHouse 小屋不存在", function(done){
			var houseInfo = {
				buildingLocation:4, houseLocation:1, finishNow:false
			}
			var route = "logic.playerHandler.upgradeHouse"
			pomelo.request(route, houseInfo, function(doc){
				doc.code.should.equal(500)
		...
		it("upgradeHouse 小屋已达到最高等级", function(done){
			var func = function(){
				var buildingInfo = {
					buildingLocation:3, houseLocation:2, finishNow:true
				}
				var route = "logic.playerHandler.upgradeHouse"
				pomelo.request(route, buildingInfo, function(doc){
					if(doc.code == 200){
		...
		it("treatSoldier 士兵不存在或士兵数量不合法2", function(done){
			treatSoldier([{name:"add", count:12}], true, function(doc){
				doc.code.should.equal(500)
				doc.message.should.equal("士兵不存在或士兵数量不合法")
				done()
			})
		...
		it("treatSoldier 士兵不存在或士兵数量不合法3", function(done){
			treatSoldier([{name:"swordsman", count:1}], true, function(doc){
				doc.code.should.equal(500)
				doc.message.should.equal("士兵不存在或士兵数量不合法")
				done()
			})
		...
		it("treatSoldier 士兵不存在或士兵数量不合法4", function(done){
			sendChat("addtreatsoldiers 5", function(doc){
				doc.code.should.equal(200)
				treatSoldier([{name:"swordsman", count:6}], true, function(doc){
					doc.code.should.equal(500)
					doc.message.should.equal("士兵不存在或士兵数量不合法")
					done()
				})
		...
		it("treatSoldier 士兵不存在或士兵数量不合法5", function(done){
			treatSoldier([{name:"swordsman", count:5}], true, function(doc){
				doc.code.should.equal(200)
				treatSoldier([{name:"swordsman", count:5}], true, function(doc){
					doc.code.should.equal(500)
					doc.message.should.equal("士兵不存在或士兵数量不合法")
		...
		it("treatSoldier 已有士兵正在治疗", function(done){
			treatSoldier([{name:"sentinel", count:5}, {name:"archer", count:5}], false, function(doc){
				doc.code.should.equal(200)
				treatSoldier([{name:"crossbowman", count:5}], false, function(doc){
					doc.code.should.equal(500)
		...
		it("treatSoldier 宝石不足", function(done){
			sendChat("gem 0", function(doc){
				doc.code.should.equal(200)
				treatSoldier([{name:"crossbowman", count:5}], true, function(doc){
					doc.code.should.equal(500)
					doc.message.should.equal("宝石不足")
					sendChat("gem 5000", function(doc){
						doc.code.should.equal(200)
		...
		it("treatSoldier 正常加速治疗", function(done){
			treatSoldier([{name:"catapult", count:5}], true, function(doc){
				doc.code.should.equal(200)
				done()
			})
		})
		...
		it("enhanceDragonEquipment 装备已到最高星级", function(done){
			sendChat("dragonequipmentstar redDragon 10", function(doc){
				doc.code.should.equal(200)
				enhanceDragonEquipment("redDragon", "crown", [{name:"moltenCrown", count:5}], function(doc){
					doc.code.should.equal(500)
		...
		it("enhanceDragonEquipment 被强化的装备不存在或数量不足2", function(done){
			enhanceDragonEquipment("redDragon", "armguardLeft", [{name:"moltenArmguard", count:30}], function(doc){
				doc.code.should.equal(500)
				doc.message.should.equal("被强化的装备不存在或数量不足")
				done()
			})
		...
		it("enhanceDragonEquipment 被强化的装备不存在或数量不足3", function(done){
			enhanceDragonEquipment("redDragon", "armguardLeft", [{name:"moltenArmguarda", count:5}], function(doc){
				doc.code.should.equal(500)
				doc.message.should.equal("被强化的装备不存在或数量不足")
				done()
			})
		...
		it("enhanceDragonEquipment 被强化的装备不存在或数量不足4", function(done){
			enhanceDragonEquipment("redDragon", "armguardLeft", [{name:"moltenArmguard", count:-1}], function(doc){
				doc.code.should.equal(500)
				doc.message.should.equal("被强化的装备不存在或数量不足")
				done()
			})
		...
		it("enhanceDragonEquipment 正常强化", function(done){
			enhanceDragonEquipment("redDragon", "armguardLeft", [{name:"moltenArmguard", count:5}], function(doc){
				doc.code.should.equal(200)
				done()
			})
		})
		...
		//			LoginPlayer(Config.deviceId2, function(doc){
		//				doc.code.should.equal(200)
		//				editAllianceBasicInfo(Config.allianceName, Config.allianceTag, "cn", "e", function(doc){
		//					doc.code.should.equal(500)
		//					doc.message.should.equal("玩家未加入联盟")
		//					done()
		//				})
		...
		//					doc.code.should.equal(200)
		//					editAllianceBasicInfo(Config.allianceName, "adfad", "cn", "e", function(doc){
		//						doc.code.should.equal(500)
		//						doc.message.should.equal("联盟名称已经存在")
		//						done()
		//					})
		...
		//			editAllianceBasicInfo("adfad", Config.allianceTag, "cn", "e", function(doc){
		//				doc.code.should.equal(500)
		//				doc.message.should.equal("联盟标签已经存在")
		//				done()
		//			})
		...
		//			editAllianceBasicInfo(Config.allianceName2, Config.allianceTag2, "cn", "e", function(doc){
		//				doc.code.should.equal(200)
		//				done()
		//			})
		//		})
		...
		//			editAllianceTerrian("grassLand", function(doc){
		//				doc.code.should.equal(500)
