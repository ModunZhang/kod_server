...
pro.upgradeAllianceBuilding = function(playerId, allianceId, buildingName, callback){
	...
	this.cacheService.findAllianceAsync(allianceId).then(function(doc){
		...
		if(_.isEqual(Consts.AllianceBuildingNames.OrderHall, buildingName)){
			...
			DataUtils.createAllianceVillage(allianceDoc, allianceData, villageTypeConfig.name, totalCount - currentCount);
		}
...
pro.attackAlliance = function(playerId, allianceId, targetAllianceId, callback){
	...
	this.cacheService.findAllianceAsync(allianceId).then(function(doc){
		...
		var now = Date.now()
		var finishTime = now + (DataUtils.getAllianceIntInit("allianceFightPrepareMinutes") * 60 * 1000 / 60)
